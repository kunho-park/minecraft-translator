...
interface MapReviewClientProps {
    mapTranslationId: string;
}

export default function MapReviewClient({ mapTranslationId }: MapReviewClientProps) {
    ...
    const fetchReviews = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/maps/reviews?mapTranslationId=${mapTranslationId}`);
            if (response.ok) {
                const data = await response.json();
                setReviews(data);
            }
        } catch (err) {
            console.error("Failed to fetch map reviews:", err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch("/api/maps/reviews", {
                method: "POST",
                ...
                <div className="mb-6">
                    ...
                    <div className="flex gap-4">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-lg border-2 transition-all ${works === true
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-lg border-2 transition-all ${works === false
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <span
                                            className={`flex items-center gap-1 text-sm ${review.works
